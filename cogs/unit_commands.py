...
class UnitCommands(commands.Cog):
    ...
    async def unit_eintritt(self, interaction: discord.Interaction, user: discord.Member, unit: discord.Role, grund: str,
        ...
        if not service or not display_service:
        ...
        # 1. Verwendung für die Hauptnachricht (soll eine Erwähnung sein)
        display_name = await display_service.get_display(result.get('user')) 
        # 2. Verwendung für den Footer (soll ein Nickname sein)
        ...
        await interaction.followup.send(f"✅ **Erfolg:** {display_name} wurde erfolgreich der Unit {result.get('unit').mention} hinzugefügt.", ephemeral=True)
    ...
    async def unit_austritt(self, interaction: discord.Interaction, user: discord.Member, unit: discord.Role, grund: str,
        ...
        if channel := self.bot.get_channel(UNIT_CHANNEL_ID):
            await channel.send(content=display_name, embed=embed)
        await interaction.followup.send(f"✅ **Erfolg:** {display_name} wurde erfolgreich aus der Unit {result.get('unit').mention} entfernt.", ephemeral=True)
    ...
    async def unit_abstieg(self, interaction: discord.Interaction, user: discord.Member, unit: discord.Role,
        ...
        # 2. Verwendung für den Footer (soll ein Nickname sein)
        requester_display_name = await display_service.get_display(interaction.user, is_footer=True)
        desc = (f"⚠️ **{display_name} wurde innerhalb der Unit {unit.mention} degradiert.**\n\n✍️ **Grund:** {grund}\n\n❌ **Alter Posten:** {alter_posten.mention}\n\n")
        if neuer_posten:
